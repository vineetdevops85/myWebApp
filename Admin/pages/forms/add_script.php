...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get Script Type model from the form
    $script_type = mysqli_real_escape_string($connection, $_POST['script_type']);

    // Check if the entry already exists
    $check_query = "SELECT * FROM script WHERE script_type = ?";
    $check_stmt = mysqli_prepare($connection, $check_query);

    if ($check_stmt) {
        mysqli_stmt_bind_param($check_stmt, "s", $script_type);
        mysqli_stmt_execute($check_stmt);
        mysqli_stmt_store_result($check_stmt);

        if (mysqli_stmt_num_rows($check_stmt) > 0) {
            $resultMessage = "Error: Script Type: $script_type already exists!";
        } else {
            // Prepare and execute the SQL statement with a prepared statement
            $insert_query = "INSERT INTO script (script_type) VALUES (?)";
            $insert_stmt = mysqli_prepare($connection, $insert_query);

            if ($insert_stmt) {
                mysqli_stmt_bind_param($insert_stmt, "s", $script_type);
                $result = mysqli_stmt_execute($insert_stmt);

                $resultMessage = $result ? "Script Type: $script_type added successfully!" : "Error: " . mysqli_error($connection);
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-open">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="add_script.php" class="nav-link active">
  ...
  <div class="content-wrapper">
          ...
          <div class="col-md-6">
            ...
            <div class="card card-info">
              <div class="card-header">
                <h3 class="card-title">Add Scripting Type</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
    ...
    <div class="card-body">
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="exampleInputEmail1" name="script_type" placeholder="Enter Scripting Type" autocomplete="off">
        </div>
    </div>
    <!-- /.card-body -->
    ...
    <div class="card-footer">
        ...
        ?>
        <button type="submit" class="btn btn-primary" name="submit_script_type">Submit</button>
    </div>
</form>
            </div>
