...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get Engineer Name model from the form
    $engg = mysqli_real_escape_string($connection, $_POST['engg']);

    // Check if the entry already exists
    $check_query = "SELECT * FROM engg WHERE engg = ?";
    $check_stmt = mysqli_prepare($connection, $check_query);

    if ($check_stmt) {
        mysqli_stmt_bind_param($check_stmt, "s", $engg);
        mysqli_stmt_execute($check_stmt);
        mysqli_stmt_store_result($check_stmt);

        if (mysqli_stmt_num_rows($check_stmt) > 0) {
          $resultMessage = "Error: Engineer Name: \"" . $engg . "\" already exists!";
        } else {
            // Prepare and execute the SQL statement with a prepared statement
            $insert_query = "INSERT INTO engg (engg) VALUES (?)";
            $insert_stmt = mysqli_prepare($connection, $insert_query);

            if ($insert_stmt) {
                mysqli_stmt_bind_param($insert_stmt, "s", $engg);
                $result = mysqli_stmt_execute($insert_stmt);

                $resultMessage = $result ? "Engineer Name: \"" . $engg . "\" added successfully!" : "Error: " . mysqli_error($connection);
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-open">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="add_engg.php" class="nav-link active">
  ...
  <div class="content-wrapper">
          ...
          <div class="col-md-6">
            ...
            <div class="card card-warning">
              <div class="card-header">
                <h3 class="card-title">Add Engineer Name</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
    ...
    <div class="card-body">
        <div class="form-group">
            <label for="exampleInputEmail1">Engineer Name</label>
            <input type="text" class="form-control" id="exampleInputEmail1" name="engg" placeholder="Enter Engineer Name" autocomplete="off">
        </div>
    </div>
    <!-- /.card-body -->
    ...
    <div class="card-footer">
        ...
        ?>
        <button type="submit" class="btn btn-success" name="submit_engg">Submit</button>
    </div>
</form>
            </div>
