...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get radio model from the form
    $radio_model = mysqli_real_escape_string($connection, $_POST['radio_model']);

    // Check if the entry already exists
    $check_query = "SELECT * FROM radio WHERE radio_model = ?";
    $check_stmt = mysqli_prepare($connection, $check_query);

    if ($check_stmt) {
        mysqli_stmt_bind_param($check_stmt, "s", $radio_model);
        mysqli_stmt_execute($check_stmt);
        mysqli_stmt_store_result($check_stmt);

        if (mysqli_stmt_num_rows($check_stmt) > 0) {
            $resultMessage = "Error: Radio Model $radio_model already exists!";
        } else {
            // Prepare and execute the SQL statement with a prepared statement
            $insert_query = "INSERT INTO radio (radio_model) VALUES (?)";
            $insert_stmt = mysqli_prepare($connection, $insert_query);

            if ($insert_stmt) {
                mysqli_stmt_bind_param($insert_stmt, "s", $radio_model);
                $result = mysqli_stmt_execute($insert_stmt);

                $resultMessage = $result ? "Radio Model $radio_model added successfully!" : "Error: " . mysqli_error($connection);
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            ...
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="scripting_entry.php" class="nav-link">
          ...
          <li class="nav-item menu-open">
            ...
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="add_radio.php" class="nav-link active">
  ...
  <div class="content-wrapper">
          ...
          <div class="col-md-6">
            ...
            <div class="card card-success">
              <div class="card-header">
                <h3 class="card-title">Add Radio Model</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
    ...
    <div class="card-body">
        <div class="form-group">
            <label for="exampleInputEmail1">Radio Model</label>
            <input type="text" class="form-control" id="exampleInputEmail1" name="radio_model" placeholder="Enter Radio Model" autocomplete="off">
        </div>
    </div>
    <!-- /.card-body -->
    ...
    <div class="card-footer">
        ...
        ?>
        <button type="submit" class="btn btn-primary" name="submit_radio">Submit</button>
    </div>
</form>
            </div>
