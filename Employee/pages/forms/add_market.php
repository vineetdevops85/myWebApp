...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get Market Name model from the form
    $market_name = mysqli_real_escape_string($connection, $_POST['market_name']);

    // Check if the entry already exists
    $check_query = "SELECT * FROM market WHERE market_name = ?";
    $check_stmt = mysqli_prepare($connection, $check_query);

    if ($check_stmt) {
        mysqli_stmt_bind_param($check_stmt, "s", $market_name);
        mysqli_stmt_execute($check_stmt);
        mysqli_stmt_store_result($check_stmt);

        if (mysqli_stmt_num_rows($check_stmt) > 0) {
            $resultMessage = "Error: Market Name: $market_name already exists!";
        } else {
            // Prepare and execute the SQL statement with a prepared statement
            $insert_query = "INSERT INTO market (market_name) VALUES (?)";
            $insert_stmt = mysqli_prepare($connection, $insert_query);

            if ($insert_stmt) {
                mysqli_stmt_bind_param($insert_stmt, "s", $market_name);
                $result = mysqli_stmt_execute($insert_stmt);

                $resultMessage = $result ? "Market Name: $market_name added successfully!" : "Error: " . mysqli_error($connection);
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-open">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="add_market.php" class="nav-link active">
  ...
  <div class="content-wrapper">
          ...
          <div class="col-md-6">
            ...
            <div class="card card-danger">
              <div class="card-header">
                <h3 class="card-title">Add Market Name</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
    ...
    <div class="card-body">
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="exampleInputEmail1" name="market_name" placeholder="Enter Market" autocomplete="off">
        </div>
    </div>
    <!-- /.card-body -->
    ...
    <div class="card-footer">
        ...
        ?>
        <button type="submit" class="btn btn-success" name="submit_market_name">Submit</button>
    </div>
</form>
            </div>
